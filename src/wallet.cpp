...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (out.nDepth < (out.tx->IsCoinStake() ? Params().COINBASE_MATURITY(int nHeight) : 10))
            continue;
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CTxDestination, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        BOOST_FOREACH (const COutput& out, vCoins) {
            ...
            if (out.tx->IsCoinStake() && out.tx->GetDepthInMainChain() < Params().COINBASE_MATURITY(int nHeight) + 1)
                continue;
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (out.tx->GetDepthInMainChain() != Params().COINBASE_MATURITY(int nHeight) + 1)
            continue;
...
int CMerkleTx::GetBlocksToMaturity() const
{
    ...
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (Params().COINBASE_MATURITY(int nHeight) + 1) - GetDepthInMainChain());
}
